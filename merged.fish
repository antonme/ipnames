...
ls dns-*.txt| awk '{$nm=substr($0,5,length($0)-8);print "cat ipv6-resolve-"$nm".txt ipv6-ext-resolve-"$nm".txt| rg -v ^:: | sort -u | sed \'s/$/\\\\/128/\' > merged/ipv6-"$name}'| bash
ls dns-*.txt| awk '{$nm=substr($0,5,length($0)-8);print "cat resolve-"$nm".txt ext-resolve-"$nm".txt| sort -u > merged/ipv4-"$name}'| bash
