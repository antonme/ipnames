...
function resolve -a name resolvers new_name concur
	parallel -j $concur --bar host -t AAAA -W 3 :::: $name :::: $resolvers| rg address | awk '$5{print $5}' | sort -u| sort -h > $new_name
end
...
if not contains -- '-ext' $argv
  echo "== Resolving names to ipv6"
  set servers "servers.txt"
else
  echo "== Resolving external names to ipv6"
  set prefixer 'ext-'
...
for name in $names
	...
	if test -z $prefixer
  	  set new_name "ipv6-resolve-"$name_array[2]
    else
  	  set new_name "ipv6-ext-resolve-"$name_array[3]
	end
